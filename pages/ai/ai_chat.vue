<template>
	<view id='box' :data-theme="state.app.theme" :class="{innerPage:innerPage}">
		...
		<view class="dialog-item-top" v-if="showChat && isShowHeader" :style="{ paddingTop: 0 + 'px' }">
			...
			<view class="dialog-item-tel">{{chatTypeDesc}} <text class="dialog-item-tel" v-if="chatType!=2">({{gpt_name}})</text></view>
		</view>
											...
											<view>
												<view class="chat-item__left-message">
													...
													<view v-if="isImage(item.answer)" class="imageBox">
														<text v-if="item.isThumb"><uni-icons type="help" color="#686efe"
																size="20"
		...
		<view v-if="chatTypeFlag=='chat' && showChat && user" class="chat-setting">
			<view style="color: rgb(153, 153, 153);">剩余{{user.number<=0?0:user.number}}次</view>
		</view>
		...
		<uni-popup ref="editChatDialog" type="dialog">
			<uni-popup-dialog ref="inputClose" mode="input" title="输入对话标题" placeholder="请输入唯一标题"
...
<script>
	...
	let answer0 = 'AIChat为您服务：\n\n'
	answer0 += '1.写文案、写论文，写小说\n\n'
	answer0 += '2.写代码、写脚本，找bug\n\n'
	answer0 += '3.翻译、写诗、作画\n\n'
	answer0 += '越详细，越精确哟！';
	...
	export default {
		...
		async onLoad(option) {
			...
			// #ifdef H5
			//当前页面才生效
			let _cur_page = 'ai/ai_chat'
			if (option && option.index) {
				_cur_page = 'ai/ai_chat_in'
			}
		...
		methods: {
			...
			selectModule2() {
				...
				if(this.chatTypeFlag=='chat' && this.showChat && this.user){
					itemList.push('切换模型及版本 '+this.gpt_name)
				}

				if(this.chatTypeFlag=='image' && this.showChat && this.user){
					...
					success: function(res) {
						switch (res.tapIndex) {
							...
							case 3:
								...
								break;
							case 4:
								if(that.chatTypeFlag=='chat' && that.showChat && that.user){
									that.changeChatVersion();
								}
								
								if(that.chatTypeFlag=='image' && that.showChat && that.user){
			...
			get_ai_chat_config() {
				this.$commonApi.get_ai_chat_configs().then((res) => {
					...
					//子页面，显示对话
					if (!this.$commonApi.isInCurrentPage('pages/ai/ai_chat')) {
						this.showChat = true
					}
			...
			async getAnswer() {
				...
				if (version == '2.0') {
					...
					//这里只是H5支持，app不支持
					if (true) {
						this.$commonApi.openai_h5(requestChatData, this.gpt_version,this.ai_chat_api_key_sec).then((res) => {
							uni.hideLoading()
							this.is_submit_question = false
							this.isStopChat = true
							if (res.code == 0) {
